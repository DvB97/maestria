(() => {
  ...
  const guideSteps = [
    ...
    { icon:'3', title:'Responde la evaluación', text:'Contesta treinta preguntas, una por vez. Verás tu progreso y podrás avanzar o regresar sin perder las respuestas.' },
  ...
  const calculateResults = () => Scoring.calculate(state.answers);
  const affinity = career => Scoring.affinity(career, state.results);
  const matchReasons = career => Scoring.reasons(career, state.results).map(areaName);
  const compareCareers = (a,b) => Scoring.compare(a,b,state.results);
  function routeGuard(route){
    const journey=['perfil','instrucciones','evaluacion','revision','procesamiento'];
    if(route==='procesamiento' && !state.results) return 'revision';
  ...
  const views = {
    inicio: () => layout(`<div class="hero"><div><div class="eyebrow">Explora con curiosidad. Decide con libertad.</div><h1>Tu futuro merece una <span>buena brújula.</span></h1><p class="lead">Una experiencia guiada para reconocer intereses, comprender tus áreas de afinidad y descubrir alternativas formativas en un espacio claro, visual y sin juicios.</p><div class="hero-actions"><button class="btn btn-primary" data-route="consentimiento">Comenzar mi recorrido →</button><button class="btn btn-secondary" data-route="ayuda">¿Cómo funciona?</button></div><div class="benefit-grid" style="margin-top:34px"><div class="benefit"><span class="icon">01</span><b>30 preguntas</b><br><span class="subtle">Recorrido breve y editable</span></div><div class="benefit"><span class="icon">02</span><b>Perfil visual</b><br><span class="subtle">Ocho áreas para explorar</span></div><div class="benefit"><span class="icon">03</span><b>Alternativas</b><br><span class="subtle">Compara y guarda opciones</span></div></div><div class="journey-strip" aria-label="Flujo principal"><span>Perfil</span><i></i><span>Evaluación</span><i></i><span>Resultados</span><i></i><span>Carreras</span></div></div><div class="visual-card" aria-label="Ejemplo visual de resultados simulados"><div class="preview-top"><div><div class="eyebrow">Vista previa del perfil</div><h2>Conoce lo que te mueve</h2></div><span class="simulated">UI-09</span></div>${[['Tecnología',86],['Análisis',78],['Creatividad',69],['Comunicación',64]].map(x=>`<div class="mini-result"><strong><span>${x[0]}</span><span>${x[1]}%</span></strong><div class="meter"><span style="width:${x[1]}%"></span></div></div>`).join('')}<div class="preview-careers"><article><b>Ingeniería de Sistemas</b><span>92% afinidad</span></article><article><b>Ciencia de Datos</b><span>88% afinidad</span></article></div><small class="subtle">Ejemplo ilustrativo con valores simulados</small></div></div>${disclaimer()}`),
  ...
  function renderCareers(){
    ...
    list.sort(sort==='name'?(a,b)=>a.name.localeCompare(b.name):compareCareers);
  ...
  function bind(){
    ...
    document.querySelectorAll('[data-answer]').forEach(b=>b.onclick=()=>{ const q=questions[state.currentQuestion]; setState({answers:{...state.answers,[q.id]:b.dataset.answer},results:null}); render(); });
    ...
    if($('#confirmAnswers')) $('#confirmAnswers').onclick=()=>{if(!Scoring.complete(state.answers))return;setState({results:calculateResults()});go('procesamiento');};
    if($('#careerGrid')){
